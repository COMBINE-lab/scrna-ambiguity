...
If you want to use conda to create the environment, simply replace the `mamba` in the above command with `conda`. The `conda_env.yml` file in the GitHub repository can be used to create the exact conda env we used to run the pipeline. Note that the yml file was generated under linux, so it might not work on other platforms with exactly those versions.
